...
def figure1_model_comparison(comparison, figures_dir):
    ...
    plt.suptitle('Fraud Detection Model Performance Comparison', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def figure2_confusion_matrices(metrics, figures_dir):
    ...
    plt.suptitle('Confusion Matrices - Fraud Detection Models', fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def figure3_detection_latency(latency, figures_dir):
    ...
    ax1.axvline(latency['mean_ms'], color='red', linestyle='--', linewidth=2, label=f"Mean: {latency['mean_ms']:.2f}ms")
    ax1.axvline(latency['median_ms'], color='green', linestyle='--', linewidth=2, label=f"Median: {latency['median_ms']:.2f}ms")
    ax1.axvline(latency['p95_ms'], color='orange', linestyle='--', linewidth=2, label=f"P95: {latency['p95_ms']:.2f}ms")
...
def figure4_precision_recall_tradeoff(figures_dir):
    ...
    ax.plot(recall, precision, color='steelblue', linewidth=3, label='Multi-Agent System')
    ...
    ax.plot(baseline_recall, baseline_precision, color='coral', linewidth=2, linestyle='--', label='Baseline (Isolation Forest)')
...
def figure5_system_architecture(figures_dir):
    ...
    # Title
    ax.text(5, 9.5, 'Multi-Agent Fraud Detection System Architecture', 
    ...
    ax.text(0.5, 2.8, 'Orchestration & UI', fontsize=12, fontweight='bold', style='italic')
