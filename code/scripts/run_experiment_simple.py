...
def generate_synthetic_data(n_transactions=5000, fraud_ratio=0.02, random_state=42):
    ...
    # Fraud transactions
    fraud_amounts = rng.lognormal(mean=4, sigma=1, size=n_fraud) * rng.uniform(3, 10, size=n_fraud)
...
def train_and_evaluate(config):
    ...
    train_df = generate_synthetic_data(n_transactions=5000, fraud_ratio=0.02, random_state=config.random_state)
    test_df = generate_synthetic_data(n_transactions=1000, fraud_ratio=0.02, random_state=config.random_state+1)
    ...
    iso_model = IsolationForest(contamination=0.02, random_state=config.random_state, n_jobs=-1)
    ...
    print(f"   Precision: {rf_metrics['precision']:.3f}, Recall: {rf_metrics['recall']:.3f}, F1: {rf_metrics['f1_score']:.3f}, AUC: {rf_metrics['auc_roc']:.3f}")
    ...
    comparison.to_csv(config.results_dir / "metrics" / "model_comparison.csv", index=False)
